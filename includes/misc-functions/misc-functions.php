...
/**
 * Set up the widget-control page that displays in the iframe for the Brick editor. Unregister all sidebars if the URL variable to unset them is set. 
 ...
 * @return   void
 */
function mp_stacks_setup_brick_sidebars(){
	
	//If we should only register the Sidebar for the Brick being edited currently
	if ( isset( $_GET['mp_stacks_widgets_setup_iframe_editor'] ) ){
		...
		if ( !empty( $GLOBALS['wp_registered_sidebars'] ) ){
			...
			//Unregister all other sidebars for now
			foreach( $GLOBALS['wp_registered_sidebars'] as $sidebar_id => $sidebar ){
				unregister_sidebar( $sidebar_id );	
			}
			
			//Get the sidebar args for the Sidebar in this Brick
			$current_sidebar = isset( $_SESSION['mp_stacks_widgets_current_sidebar'] ) ? $_SESSION['mp_stacks_widgets_current_sidebar'] : NULL;
			...
			if ( !empty( $current_sidebar ) ){
					...
					if ( empty( $current_sidebar['id'] ) ){
						$current_sidebar['id'] = sanitize_title( $current_sidebar['name'] );
					}
								
					//Register the sidebar into this iframe so that we can edit it.
					register_sidebar( $current_sidebar );
	...
	//If we are not editing a Brick with a sidebar right now, register all the Brick Sidebars
	else{
...
add_action( 'widgets_init', 'mp_stacks_setup_brick_sidebars', 100 );
...
function mp_stacks_ajax_set_sidebars_transient(){
	...
	//Get the existing MP Stacks + Widgets Sidebars saved into Bricks
	$mp_stacks_sidebars = get_option( 'mp_stacks_sidebar_args' ); 
	
	//Add this Sidebar into the list of sidebars saved into Bricks
	...
	//Save the MP Stacks + Widgets Sidebars saved into Bricks with this new one added.
	update_option( 'mp_stacks_sidebar_args', $mp_stacks_sidebars );
	
	//Set the Session variable to let the iframe know which sidebar we want to edit.
	$_SESSION['mp_stacks_widgets_current_sidebar'] = $mp_stacks_sidebars[$sidebar_id];
	
	//Give a "Manage Widgets" button to the meta field for the user to click since their sidebar will now be available 
	echo json_encode( array( 
		'iframe' => '<iframe src="' . mp_core_add_query_arg( array( 'mp-stacks-minimal-admin' => true, 'mp_stacks_widgets_setup_iframe_editor' => true ), admin_url( 'widgets.php' ) ) . '" target="_blank" width="100%" height="500px" frameborder="0" scrolling="no" onload=\'javascript:mp_stacks_resizeIframe(this);\'>' . __( 'Manage Widgets', 'mp_stacks_widgets' ) . '</a>',
...
/**
 * ReCheck all Bricks for sidebars and add those to the Option in the Options table containing all the Sidebars in Bricks.
 *
 * @access   public
 * @since    1.0.0
 * @return   void
 */
function mp_stacks_widgets_recheck_all_bricks_for_sidebars(){
	
	if ( !isset( $_GET['mp_stacks_widgets_recheck_all_bricks_for_sidebars'] ) ){
		return;	
	}
...
add_action( 'admin_init', 'mp_stacks_widgets_recheck_all_bricks_for_sidebars' );